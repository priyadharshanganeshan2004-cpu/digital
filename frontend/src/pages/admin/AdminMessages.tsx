...
export default function AdminMessages() {
    ...
    const { data: messages, isLoading: loadingMessages } = useQuery<Message[]>({
        queryKey: ['admin-messages', activeConversation],
        queryFn: async () => {
    ...
    const sendMutation = useMutation({
        ...
        onSuccess: () => {
            setNewMessage('');
            queryClient.invalidateQueries({ queryKey: ['admin-messages', activeConversation] });
    ...
    return (
        <div className="space-y-4">
            <div>
                <h2 className="text-xl font-heading font-bold text-dark-900">Support Desk</h2>
                <p className="text-sm text-dark-400 mt-1">Converse with your agency clients in real-time</p>
            </div>
                ...
                <div className="flex h-full">
                    ...
                    <div className="w-72 border-r border-gray-100 flex flex-col">
                        <div className="p-4 border-b border-gray-100">
                            <h3 className="font-heading font-semibold text-dark-900 text-sm">Active Chats</h3>
                        </div>
                        <div className="flex-1 overflow-y-auto">
                            {loadingConvos ? (
                                ...
                                <div className="p-6 text-center">
                                    ...
                                    <p className="text-xs text-dark-400">No active client threads yet</p>
                                </div>
                            ) : (
                        ...
                        {!activeConversation ? (
                            <div className="flex-1 flex items-center justify-center">
                                <div className="text-center">
                                    ...
                                    <p className="text-dark-400">Select a client chat to view messages</p>
                                </div>
                            </div>
                        ) : (
                            <>
                                ...
                                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                                    {loadingMessages ? (
                                        ...
                                        <div className="text-center py-8">
                                            <p className="text-sm text-dark-400">No messages in this thread.</p>
                                        </div>
                                    ) : (
                                        messages.map((msg) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div className={`max-w-[70%] px-4 py-2.5 rounded-2xl text-sm ${isMe
                                                        ...
                                                        <span className="text-[9px] block font-semibold text-white/70 mb-0.5">{msg.sender.name}</span>
                                                        <p>{msg.content}</p>
