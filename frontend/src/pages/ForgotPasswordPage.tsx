...
export default function ForgotPasswordPage() {
    ...
    const onSubmit = async (data: { email: string }) => {
        try {
            setError('');
            await emailApi.requestOtp(data.email);
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md bg-white rounded-3xl p-8 shadow-xl shadow-gray-100/50 border border-gray-100">
                    ...
                    {isSubmitted ? (
                        <div className="text-center">
                            ...
                            <p className="text-dark-500 text-sm mb-4">We've sent a password reset OTP to {submittedEmail || 'your email address'}.</p>
                            <p className="text-dark-400 text-xs mb-6">Open the reset page and enter the code from your inbox.</p>
                            <Link to="/login" className="btn-primary w-full">Back to Sign In</Link>
                            <Link to={`/reset-password?email=${encodeURIComponent(submittedEmail)}`} className="mt-3 block text-sm text-primary-600 font-semibold">Go to reset page</Link>
                        </div>
                    ) : (
                        <>
